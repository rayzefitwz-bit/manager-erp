...
export const Finance = () => {
  const { transactions, addTransaction, immersiveClasses, team, payCommission } = useApp();
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (formData.amount && formData.description) {
      ...
      if (formData.category === 'Comissões' && formData.memberId) {
        const member = team.find(m => m.id === formData.memberId);
        if (member) {
          payCommission(member.id, Number(formData.amount), member.role === 'PROFESSOR' ? 'TEACHER' : 'SELLER', member.name);
          setFormData({ type: 'INCOME', amount: '', description: '', category: 'Geral', classLocation: '', memberId: '' });
          setShowForm(false);
          return;
        }
      }
        ...
        <div className="bg-white p-6 rounded-xl shadow-md border border-blue-100 animate-fade-in">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              {formData.category === 'Comissões' && (
                <div className="animate-fade-in">
                  <label className="block text-xs font-bold text-indigo-600 uppercase mb-1">
                    Vendedor / Professor
                  </label>
                  <select
                    className="w-full border-2 border-indigo-200 p-2.5 rounded-lg text-sm focus:border-indigo-500 outline-none"
                    value={formData.memberId}
                    onChange={e => setFormData({ ...formData, memberId: e.target.value })}
                    required
                  >
                    <option value="">Selecione quem está recebendo...</option>
                    {team.filter(m => m.role !== 'ADMIN').map(member => (
