...
export const KnowledgeBase = () => {
  ...
  // Refactored to support both "Image Source" and "Embed/Preview" modes
  const getPreviewUrl = (url: string, mode: 'source' | 'embed' = 'source') => {
    if (url.includes('drive.google.com')) {
      const id = getDriveFileId(url);
      if (id) {
        // Source = Download/View raw image
        // Embed = HTML Preview Page
        return mode === 'embed'
  ...
  const getMainPreviewType = (url: string): 'IMAGE' | 'PDF' | 'YOUTUBE' | 'NONE' => {
    if (!url) return 'NONE';
    const lower = url.toLowerCase();

    // Google Drive (Generic) - Check this FIRST to force Iframe preview for all Drive links (safer)
    if (lower.includes('drive.google.com')) {
      return 'PDF';
    }

    // Images
    if (lower.match(/\.(jpeg|jpg|gif|png|webp|bmp|svg)($|\?)/) ||
      lower.includes('images.unsplash.com') ||
      lower.includes('GOOGLE_USER_CONTENT_ID_PLACEHOLDER') || // Placeholder for complex logic if needed
      lower.includes('imgur.com')) return 'IMAGE';
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-[70] p-4 animate-fade-in">
          <div className="bg-white w-full max-w-4xl max-h-[90vh] rounded-2xl shadow-2xl flex flex-col overflow-hidden">
            ...
            <div className="flex-1 overflow-y-auto p-8 custom-scrollbar">
              {/* Main Preview Section (Sync URL / Main Link) */}
              {(() => {
                const targetUrl = viewingItem.syncUrl || viewingItem.link;
                const previewType = targetUrl ? getMainPreviewType(targetUrl) : 'NONE';

                if (previewType !== 'NONE' && targetUrl) {
                  return (
                    <div className="mb-8 rounded-xl overflow-hidden border border-gray-200 shadow-sm bg-gray-900 flex justify-center">
                      {previewType === 'IMAGE' && (
                        <img src={getPreviewUrl(targetUrl, 'source')} alt="Preview" className="max-h-[500px] object-contain" />
                      )}
                      {previewType === 'PDF' && (
                        <iframe src={getPreviewUrl(targetUrl, 'embed')} className="w-full h-[600px] bg-white" title="Document Preview" />
                      )}
                      {previewType === 'YOUTUBE' && (
                        <iframe
                          width="100%"
                          height="450"
                          src={`https://www.youtube.com/embed/${targetUrl.includes('v=') ? targetUrl.split('v=')[1].split('&')[0] : targetUrl.split('/').pop()}`}
                      ...
                      <div key={idx} className="bg-white border border-gray-200 rounded-xl overflow-hidden shadow-sm hover:shadow-md transition-shadow">
                        {att.type === 'image' ? (
                          <div className="group relative">
                            <img src={getPreviewUrl(att.url, 'source')} alt={att.name} className="w-full h-[300px] object-contain bg-gray-50" />
                          ...
                          <div className="bg-gray-900 h-[300px] flex items-center justify-center relative group">
                            {att.url.includes('drive.google.com') ? (
                              <iframe
                                src={getPreviewUrl(att.url, 'embed')}
